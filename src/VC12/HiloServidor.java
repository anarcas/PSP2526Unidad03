    ...
    public void run() {
        ...
        try (Socket s = this.socket; BufferedReader entrada = new BufferedReader(new InputStreamReader(s.getInputStream())); PrintWriter salida = new PrintWriter(s.getOutputStream(), true)) {
            ...
            if (peticion != null && (peticion.startsWith("GET") || peticion.startsWith("POST"))) {
                ...
                if (ruta.equals("/")) {
                    //respuestaHTML = construirRespuesta(OK, PaginasHTML.html_index);
                    respuestaHTML = construirRespuesta(OK, PaginasHTML.html_index(""));
                } else if (ruta.startsWith("/formulario") && peticion.startsWith("GET")) {
                    respuestaHTML = construirRespuesta(OK, PaginasHTML.html_formulario);
                } else if (ruta.startsWith("/formulario") && peticion.startsWith("POST")) {
                    System.out.println("Linea separación: " + cuerpo.toString());
                    String nombre = cuerpo.toString().split("&")[0].split("=")[1].toUpperCase();
                    System.out.println("Nombre: " + nombre);
                    //respuestaHTML = construirRespuesta(OK, PaginasHTML.html_index;
                    respuestaHTML = construirRespuesta(OK, PaginasHTML.html_index(nombre));
                } else {
